...
function search()
{
    ...
    for(let i = 1; i <= entryNum; i ++)
    {
        ...
        document.getElementById("table").innerHTML += `<tr><td>${i}</td><td>${auton_shots}</td><td>${auton_beacons}</td><td>${park}</td><td>${teleop_shots}</td><td>${teleop_beacons}</td><td>${cap_ball}</td><td class="notes">${note}</td></tr>`
    }
}